...
namespace ConsoleApplication
{
    public class Student
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (this.Name.Equals(stud.Name) && this.Jmbag.Equals(stud.Jmbag) && this.Gender.Equals(stud.Gender)) return true;
            return false;
        }
        ...
        {
            if (prvi.Name.Equals(drugi.Name) && prvi.Jmbag.Equals(drugi.Jmbag) && prvi.Gender.Equals(drugi.Gender)) return false;
            return true;
        }
        public static bool operator ==(Student prvi, Student drugi)
        {
            if(prvi.Name.Equals(drugi.Name)&&prvi.Jmbag.Equals(drugi.Jmbag)&&prvi.Gender.Equals(drugi.Gender))return true;
            return false;
        }
    }    public enum Gender
    {
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Student[] croatianStudentsOnMultipleUniversities = universities.SelectMany(uni => uni.Students).GroupBy(stud => stud).Where(st => st.Count() > 1).Select(x=>x.Key).ToArray();
            Student[] studentsOnMaleOnlyUniversities = universities.SkipWhile(uni => !uni.Students.TakeWhile(stud => stud.Gender.Equals("Female")).Equals(null)).SelectMany(univ => univ.Students).ToArray();
